    ...
    switch (fork()) {
        ...
        case 0:     /* child process */
            ...
            for (int i = 0; i < times; ++i) {
                if(!writeEh) write(pipefd_1[1], &ch, 1);
                else read(pipefd_1[0], &ch, 1);
            }
        ...
        default:    /* parent process */
            ...
            if(0>clock_gettime(clk_id, &start)) return perror("clock_gettime"),1;
            for (int i = 0; i < times; ++i) {
                if(writeEh) write(pipefd_1[1], &ch, 1);
                else read(pipefd_1[0], &ch, 1);
            }
            if(0>clock_gettime(clk_id, &stop)) return perror("clock_gettime"),1;
